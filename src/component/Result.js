...
return (<>
    ...
    <table>
        ...
        {orders.map((order)=>(<tr><td>{order.type}</td><td>{order.size}</td><td>{order.count}</td><td><Calculation type={order.type} size={order.size} count={order.count} setTotal={setTotal} UserContext={UserContext}/></td></tr>))}
